...
''' The following code by Chris Billington, from inofify_simple: '''
...
'''
----------------------
End of inotify_simple, now to the actual script.
'''

inotify = INotify()
w = inotify.add_watch('/dev/disk/by-id', flags.CREATE)
...
while True:
    ...
    for event in inotify.read():
        devicename = event[3]
        device = join('/dev/disk/by-id/', devicename)
